...
insert_sql = 'insert into video_log_copy_1(`datetime`,url, vid) values (%s, %s, %s)'

def parse_url_time(filedir):
    filelist = os.listdir(filedir)
    for path in filelist:
        filepath = filedir + '/' + path
        if not os.path.isfile(filepath) or not filepath.endswith('.gz'):
            continue
        print("--"+filepath)
...
def _parse_url_time(filepath):
    ...
    with GzipFile(filepath, 'r') as fi:
        ...
        for line in lines:
            ...
            if len(url_arr) == 0:
                continue
            url = url_arr[0].rstrip('HTTPS"').rstrip('HTTP"').strip()
            if '.mp4' in url:
                try:
                    vid = url[34:url.index('/data')]
                except:
                    vid = ''
                row_sql.append((ti, url, vid))
                data_num += 1
                if data_num >= 1000:
                    do_insert(row_sql)
                    row_sql = []
